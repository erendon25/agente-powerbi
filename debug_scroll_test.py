...
async def scroll_to_find(page, text_to_find):
    label = page.locator(f"text='{text_to_find}'").last
    ...
    for frame in page.frames:
        try:
            # specifically "Top Places" has a chart / matrix
            # Let's find any visual container holding a scrollbar
            visuals = frame.locator("visual-container")
            for i in range(await visuals.count()):
                visual = visuals.nth(i)
                if await visual.is_visible():
                    # try to see if it's the one with text we want, we can just hover and wheel
                    await visual.hover(force=True)
                    for _ in range(10):
                        if await label.is_visible():
                            return True
                        await page.mouse.wheel(0, 500)
                        await page.wait_for_timeout(400)
                    # Scroll back up to reset for the next search
                    for _ in range(10):
                        await page.mouse.wheel(0, -500)
                        await page.wait_for_timeout(100)
        except Exception as e:
            pass
    return await label.is_visible()
...
async def main():
    async with async_playwright() as p:
        ...
        for st in stores_to_check:
            found = await scroll_to_find(page, st)
            print(f"[{st}] Found = {found}")
            
            if found:
                label = page.locator(f"text='{st}'").last
